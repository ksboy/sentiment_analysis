...
from utils import convert_examples_to_features, ChnSentiCorpProcessor
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results, _ = evaluate(args, model, tokenizer, processor, mode="dev")
...
def evaluate(args, model, tokenizer, processor, mode, prefix=""):
    eval_dataset = load_and_cache_examples(args, tokenizer, processor, mode=mode)
    ...
    return results, [label_map[label] for label in preds]

def predict(args, model, tokenizer, processor, mode, prefix=""):
    eval_dataset = load_and_cache_examples(args, tokenizer, processor, mode=mode)
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, tokenizer, processor, mode="train")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, predictions = evaluate(args, model, tokenizer, processor, mode="dev", prefix=checkpoint)
            # Save results
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = AutoModelForTokenClassification.from_pretrained(checkpoint)
        model.to(args.device)
        result, predictions = evaluate(args, model, tokenizer, processor, mode="test")
        # Save results
